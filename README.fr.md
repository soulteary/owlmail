...
[![MailDev Workflows](https://img.shields.io/badge/MailDev-Workflow%20Compatibility-blue.svg)](./docs/fr/Comparison-and-Migration.md)
...
**Tags disponibles :**
- `0.8.0` - Tag de version exact ; `0.8` et `0` évoluent avec les versions ultérieures de ces séries
- `sha-<commit>` - Image d'un SHA court précis (par exemple `sha-e3d2cfc`)
...
| `-mail-index-path` | `OWLMAIL_MAIL_INDEX_PATH` | - | Chemin facultatif d’un index SQLite reconstructible ; les fichiers EML restent la source de vérité |
...
### Modes d’authentification SMTP entrante

Sans `-smtp-user` ni `-smtp-password`, OwlMail utilise le mode **NO AUTH** : la
remise sans authentification est autorisée et PLAIN/LOGIN acceptent aussi des
...
> utilisables sans TLS pour le développement ; activez TLS avant d’utiliser de
> vrais identifiants et isolez l’écoute SMTP.
...
[guide de migration](./docs/fr/Comparison-and-Migration.md).
...
Les clients REST existants peuvent activer explicitement la façade MailDev,
désactivée par défaut. Les nouvelles intégrations doivent utiliser l'API OwlMail
versionnée. La façade n'ajoute pas de compatibilité Socket.IO :

```bash
# Client REST MailDev existant
OWLMAIL_MAILDEV_REST_COMPAT=true ./owlmail
curl http://localhost:1080/api/email

# Nouvelle intégration OwlMail
...
For detailed migration guide, see: [OwlMail × MailDev: Full Feature & API Comparison and Migration White Paper](./docs/fr/Comparison-and-Migration.md)
...
- [Notes de version OwlMail 0.7.0](./docs/en/Release-0.7.0.md) ([中文](./docs/zh-CN/Release-0.7.0.md))
- [Journal des modifications](./CHANGELOG.md)
- [OwlMail × MailDev: Full Feature & API Comparison and Migration White Paper](./docs/fr/Comparison-and-Migration.md)
