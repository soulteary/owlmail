...
`v0.10.0` は現在の安定版であり、セキュリティリリースです。Web UI、REST API、
WebSocket ストリーム、読み取り専用 MCP エンドポイントは、既定でブラウザの
`Origin` ヘッダーを検証するようになりました。捕捉したメールは誰でも一覧できる
ディレクトリには書き込まれなくなり、暗黙 TLS リスナーのポートは、しばしば bind
できなかった特権ポート固定ではなく設定可能になりました。runtime API と保存形式は
0.9.0 と互換ですが、アップグレード時に見える変更が 3 つあります。デプロイ前に
リリースノートをご確認ください。

以下の例は `ghcr.io/soulteary/owlmail:0.10.0` に固定しています。
再現可能な CI では完全なバージョンまたは
`ghcr.io/soulteary/owlmail@sha256:<digest>` を使用してください。
[0.10.0 リリースノート](./docs/en/Release-0.10.0.md)も参照してください。
...
**利用可能なタグ：**
- `0.10.0` - 正確なリリースタグ。`0.10` と `0` は同じ系列の後続リリースで更新
- `sha-<commit>` - 特定の短いコミット SHA のイメージ（例：`sha-8d3445d`）
...
| `-smtp-read-timeout` | `OWLMAIL_SMTP_READ_TIMEOUT` | 10s | SMTP コマンドおよび DATA の読み取りタイムアウト |
| `-smtp-write-timeout` | `OWLMAIL_SMTP_WRITE_TIMEOUT` | 10s | SMTP 応答の書き込みタイムアウト |
| `-smtp-max-recipients` | `OWLMAIL_SMTP_MAX_RECIPIENTS` | 50 | 1 メッセージあたりの最大受信者数 |
...
| `-mcp-allowed-origins` | `OWLMAIL_MCP_ALLOWED_ORIGINS` | - | OwlMail 自身のオリジンに加えて `/mcp` で受け入れるブラウザーオリジン。`*` はオリジン検証を無効化 |
...
| `-smtps-port` | `OWLMAIL_SMTPS_PORT` | 465 | TLS 有効時の暗黙 TLS（SMTPS）リスナーのポート。`0` を指定すると SMTPS リスナーを起動しない |
| `-log-level` | `MAILDEV_VERBOSE` / `MAILDEV_SILENT` / `OWLMAIL_LOG_LEVEL` | normal | Log level |
| `-mailcatcher-rest-compat` | `OWLMAIL_MAILCATCHER_REST_COMPAT` | false | オプションの MailCatcher REST 互換 API を有効化 |
...
For detailed migration guide, see: [OwlMail × MailDev × MailCatcher × Mailpit: 機能・API・移行ガイド](./docs/ja/Comparison-and-Migration.md)
...
- [OwlMail 0.10.0 リリースノート](./docs/en/Release-0.10.0.md) ([中文](./docs/zh-CN/Release-0.10.0.md))
...
- [変更履歴](./CHANGELOG.md)
- [OwlMail × MailDev × MailCatcher × Mailpit: 機能・API・移行ガイド](./docs/ja/Comparison-and-Migration.md)
