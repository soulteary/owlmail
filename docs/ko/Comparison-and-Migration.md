# OwlMail × MailDev × MailCatcher × Mailpit: 기능, API 및 마이그레이션 가이드
...
OwlMail, MailDev, MailCatcher 및 Mailpit은 기본 개발 워크플로를 공유하지만 **프로토콜이 동일하지
않으며 검증 없이 교체할 수 없습니다**. API 접두사, 응답 형식, 읽음 상태 및
실시간 프로토콜이 다릅니다. 현재 범위는
[API 참조](../en/API-Reference.md)를 확인하세요.

OwlMail 0.10.0은 기본적으로 비활성화된 읽기 전용 MCP 인터페이스를 Streamable HTTP와 stdio로 제공합니다. MailDev 3의 MCP 범위는 더 넓고 MailCatcher와 Mailpit에는 내장 MCP가 없습니다. Mailpit용 호환 facade도 없습니다. 기본적으로 비활성화된 `-maildev-rest-compat` 옵션은 현재 MailDev REST 경로를
