...
`v0.10.0`。GitHub Release 二进制与容器镜像必须从同一个标签构建。
...
对于 0.10.0，还需要确认浏览器 `Origin` 校验在 Web、API、WebSocket 与 MCP 各界面
上都经过验证（包括文档化的 `*` 关闭开关），SMTPS 监听无法绑定的配置会直接启动失败
而不是静默继续，模糊测试种子语料可干净重放，并且所有当前版本示例均由 `VERSION`
派生或已在打标签前完成更新。
...
对于 `v0.10.0`，检查 `0.10.0`、`0.10`、`0` 标签和两个目标架构。`main`、`latest`
