...
打开 `http://localhost:1080/webhooks`，或点击收件箱中的 **Webhook 配置**，即可
生成版本 1 配置。内置中英文编辑器也支持通过文件选择、拖放或粘贴文本导入已有
JSON，按照文档中的目标限制进行校验，并复制或下载规范化结果。

编辑器只在本地工作：它不会上传配置，也不能修改正在运行的服务。下载 JSON 后，
请把文件放置或挂载到 OwlMail 可读位置，通过 `-webhook-config` 或
`OWLMAIL_WEBHOOK_CONFIG` 指定它，然后重启 OwlMail。类似
`${OWLMAIL_WEBHOOK_SECRET}` 的值会保留为占位符，必须在服务端启动环境中提供。
...
| `name` | 是 | 仅用于日志的安全标识，最长 100 个 UTF-8 字节且不能包含换行；完整 URL 和配置密钥不会写入日志。 |
...
每个已存储邮件事件都会先同步写入邮件目录下的 `.owlmail-webhook-outbox`，本地提交
完成后事件交接即返回。后台 worker 再把条目转交给 Redis 或内存队列，只有队列接收
成功才删除本地文件；队列临时失败时，outbox 条目会保留并重试。需要跨重启恢复时，
必须配置持久化的 `-mail-directory`，因为默认的进程专属临时目录不会被下个进程复用。

设置 `OWLMAIL_WEBHOOK_REDIS_URL` 后，consumer group worker 仅在所有匹配目标成功后
确认并删除 Stream 条目；进程崩溃遗留的 pending 条目会在 30 秒后被重新认领。目标
耗尽自身重试后，包含投递错误的记录会写入 `<prefix>:dead-letter`，随后移除活动条目。
Redis 必须在启动时可访问；启动后的 Redis 故障由本地 outbox 暂存，直到交接成功。

该语义是“至少一次”：若接收端已经成功、但进程在确认 Redis 前崩溃，重启后会
重复投递。接收端应使用稳定的 `X-OwlMail-Delivery-ID` 去重；仅使用邮件 ID 无法
区分同一邮件未来可能产生的多个事件。当前 consumer 配置面向每个前缀单个活动
OwlMail 实例。

退出时会先停止 SMTP 入口和新的队列交接，在 `-webhook-shutdown-timeout` 内排空
outbox、排队任务及执行中的投递，超时后再取消共享上下文。未配置 Redis 时，本地
文件只覆盖内存队列接收前的故障；条目转入内存队列后不再具备跨重启恢复能力。

## 并发与背压

`-webhook-max-concurrency` 限制所有邮件共享的并发目标 HTTP 请求数。默认值 `8`
适合本地开发和小型 CI 环境。估算初始值时，可以用“峰值每秒目标请求数 × 接收端
p95 响应秒数”，然后向上取整。

SMTP `DATA` 命令只等待本地 outbox 同步提交，不等待 Redis append、可用投递槽位或
目标 HTTP 请求。同一邮件命中的目标会独立调度，因此慢目标不会串行阻塞其他目标。
...
`[a-z]` 等字符组以及 `\` 转义。语法沿用 Go 的 `path.Match`，但过滤对象是任意文本
而非路径，因此 `*`、`?` 和字符组都可以匹配 `/`。
...
2. SMTP 确认前，`new` 事件会同步写入本地 outbox。HTTP 投递仍是异步的，因此目标
   缓慢或失败不会改变已保存邮件和最终的 SMTP 接收结果；HTTP 并发耗尽也不会阻塞
   outbox 交接；
3. 命中的目标独立调度，进程级上限按每个目标 HTTP 请求获取；
...
Redis 模式提供持久化投递队列、重启恢复与死信 Stream。未配置 Redis 时，本地
outbox 仅持久化到内存队列接收任务为止；重试耗尽的任务会记录错误，重启后不会
重放。关闭时会在配置的期限内排空已接收任务，详见
[关闭与投递保证](./Operations.md#关闭与投递保证)。
...
- 重试可能造成重复请求，执行非幂等动作前应使用 `X-OwlMail-Delivery-ID` 去重；
  邮件 ID 不是唯一的投递标识。
- 默认请求体可能同时包含纯文本和 HTML 正文；接收端只需要标题或验证码时，
  应使用自定义模板减少数据；
- Webhook 转发是“至少一次”而非“恰好一次”；需要跨重启恢复时，应使用 Redis
  与支持幂等处理的持久化下游服务。
