...
2. `new` 事件异步启动 HTTP 投递，因此目标缓慢或失败不会改变已保存邮件和最终的
   SMTP 接收结果；如果进程级并发限制已经耗尽，SMTP 确认会按上文说明等待槽位；
...
记录失败，但重启后不会再次发送该事件。进程关闭时不会等待正在进行的 Webhook
投递完成，详见[关闭与投递保证](./Operations.md#关闭与投递保证)。
