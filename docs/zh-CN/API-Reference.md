...
失败任务只暴露受限的 `errorCategory`，不会返回下游原始错误。配置持久邮件目录后，
任务会在入队前原子写入 `.owlmail-meta/relay-jobs`，重启时会重新提交尚未完成的任务。
连接与超时错误使用指数退避和有界抖动，最多尝试三次；认证等已分类的永久错误立即
进入终态。任务响应会暴露尝试次数和下一次计划时间，但不包含下游原始错误。
完成记录会在 24 小时后过期。状态存储最多保存 1000 条；完成记录在完成后
至少保护一分钟，此后可能因容量压力提前淘汰，执行中任务永不淘汰。如果所有槽位都
在执行或仍处于保护期，新请求返回 `503` 和 `Retry-After: 1`。显式 `relayTo` 在
创建状态记录前限制为 1024 个 UTF-8 字节。持久化故障会使新中继请求返回 `503`，
不会静默接受易失任务。恢复语义为“至少一次”：若下游已接收但终态尚未落盘时崩溃，
重启后可能产生一次重复尝试。历史 `/email` 别名保持原响应行为；
