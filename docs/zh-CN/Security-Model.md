...
OwlMail 写入的持久化产物都带有收紧的权限位：邮件正文、附件、元数据 sidecar、
Webhook 发件箱、Relay Job 日志与 SQLite 索引为 `0600`；每封邮件的附件目录与
Relay Job 目录为 `0700`；邮件目录与隔离目录为 `0750`。因此属主所在的组只能列出
邮件目录，既读不到任何捕获的邮件，也进不去单封邮件的附件目录，主机上其他本地账号
两者都做不到。组仍然保留列目录的能力，所以不受信任的同机租户仍需要独立账号或
独立容器。

上述文件权限都由显式 `chmod` 固定，而不是沿用 `os.CreateTemp` 请求的权限——后者
会被 OwlMail 启动时的 umask 削减。目录权限来自创建调用本身，更严格的 umask 只会
让它更窄。

这些权限位只作用于 OwlMail 新建的产物。已存在的目录权限不会被修改，升级不会重写
早期版本留下的邮件目录，也不会覆盖你有意设置的挂载卷权限。需要收紧既有目录时自行
执行 `chmod 0750 "$MAIL_DIR"`，再执行
`find "$MAIL_DIR" -mindepth 1 -type d -exec chmod 0700 {} +` 与
