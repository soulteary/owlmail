# OwlMail × MailDev × MailCatcher × Mailpit : guide des fonctionnalités, de l'API et de la migration
...
OwlMail, MailDev, MailCatcher et Mailpit couvrent les mêmes workflows de développement essentiels,
...
OwlMail 0.10.0 fournit des interfaces MCP en lecture seule, désactivées par défaut, via Streamable HTTP et stdio. MailDev 3 propose un workflow MCP plus large ; MailCatcher et Mailpit n'intègrent pas MCP. Aucune façade de compatibilité n'existe pour Mailpit. L'option `-maildev-rest-compat`, désactivée par défaut, expose les routes REST
