# OwlMail Documentation

Welcome to the OwlMail documentation directory. This directory contains technical documentation, migration guides, and API reference materials in multiple languages.

## 📸 Preview

![OwlMail Preview](../.github/assets/preview.png)

## 🎥 Demo Video

![Demo Video](../.github/assets/realtime.gif)

## 🌍 Languages / 语言 / Sprachen / Langues / Lingue / 言語 / 언어

Select your preferred language:

- [English](#english) | [简体中文](#简体中文) | [Deutsch](#deutsch) | [Français](#français) | [Italiano](#italiano) | [日本語](#日本語) | [한국어](#한국어)

---

## English

📖 **[Read Documentation in English →](./en/README.md)**

### Available Documents

- [Integration Testing](./en/Integration-Testing.md) and [CI Quickstart](./en/CI-Quickstart.md)
- [AI Agent Testing](./en/AI-Agent-Testing.md) and [MCP Reference](./en/MCP-Reference.md)
- [Testing Recipes](./en/Testing-Recipes.md)
- [Architecture](./en/Architecture.md) and [Security Model](./en/Security-Model.md)
- [Runnable JavaScript, Go, Python, and Compose examples](../examples/testing/README.md)
- [OwlMail 0.10.0 Release Notes](./en/Release-0.10.0.md)
- [OwlMail 0.9.0 Release Notes](./en/Release-0.9.0.md)
- [API Reference](./en/API-Reference.md)
- [OpenAPI 3.1 Contract (JSON)](../openapi/openapi.json) / [YAML](../openapi/openapi.yaml)
- [Operations and Troubleshooting](./en/Operations.md)
- [Sendmail-compatible CLI](./en/Sendmail.md)
- [Release Process](./en/Releasing.md)
- [OwlMail × MailDev × MailCatcher × Mailpit - Feature, API, and Migration Guide](./en/Comparison-and-Migration.md)
- [API Refactoring Record (historical)](./en/internal/API_Refactoring_Record.md)
- [Webhook Forwarding](./en/Webhook-Forwarding.md)
- [Webhook Scenario Examples](../examples/webhooks/README.md)

---

## 简体中文

📖 **[阅读中文文档 →](./zh-CN/README.md)**

### 可用文档

- [集成测试](./zh-CN/Integration-Testing.md)与 [CI 快速入门](./zh-CN/CI-Quickstart.md)
- [AI Agent 测试](./zh-CN/AI-Agent-Testing.md)与 [MCP 参考](./zh-CN/MCP-Reference.md)
- [测试配方](./zh-CN/Testing-Recipes.md)
- [架构](./zh-CN/Architecture.md)与[安全模型](./zh-CN/Security-Model.md)
- [JavaScript、Go、Python 与 Compose 可运行示例](../examples/testing/README.zh-CN.md)
- [OwlMail 0.10.0 发布说明](./zh-CN/Release-0.10.0.md)
- [OwlMail 0.9.0 发布说明](./zh-CN/Release-0.9.0.md)
- [API 参考](./zh-CN/API-Reference.md)
- [OpenAPI 3.1 合约（JSON）](../openapi/openapi.json) / [YAML](../openapi/openapi.yaml)
- [运维与排障](./zh-CN/Operations.md)
- [Sendmail 兼容 CLI](./zh-CN/Sendmail.md)
- [发布流程](./zh-CN/Releasing.md)
- [OwlMail × MailDev × MailCatcher × Mailpit - 功能、API 与迁移指南](./zh-CN/Comparison-and-Migration.md)
- [API 重构记录（历史资料）](./zh-CN/internal/API_Refactoring_Record.md)
- [Webhook 消息转发](./zh-CN/Webhook-Forwarding.md)
- [Webhook 场景示例](../examples/webhooks/README.zh-CN.md)

---

## Deutsch

📖 **[Dokumentation auf Deutsch lesen →](./de/README.md)**

### Verfügbare Dokumente

- [OwlMail × MailDev × MailCatcher × Mailpit - Funktions-, API- und Migrationsleitfaden](./de/Comparison-and-Migration.md)
- [API-Refactoring-Aufzeichnung](./de/internal/API_Refactoring_Record.md)
- Gemeinsame Referenzen: [0.10.0 Release Notes (English)](./en/Release-0.10.0.md), [API (English)](./en/API-Reference.md), [OpenAPI JSON](../openapi/openapi.json) / [YAML](../openapi/openapi.yaml), [Operations (English)](./en/Operations.md), [Webhook (English)](./en/Webhook-Forwarding.md)
- [Sendmail-kompatible CLI](./de/Sendmail.md)

---

## Français

📖 **[Lire la documentation en français →](./fr/README.md)**

### Documents disponibles

- [OwlMail × MailDev × MailCatcher × Mailpit : guide des fonctionnalités, de l'API et de la migration](./fr/Comparison-and-Migration.md)
- [Enregistrement de la refactorisation de l'API](./fr/internal/API_Refactoring_Record.md)
- Références partagées : [Notes de version 0.10.0 (English)](./en/Release-0.10.0.md), [API (English)](./en/API-Reference.md), [OpenAPI JSON](../openapi/openapi.json) / [YAML](../openapi/openapi.yaml), [Operations (English)](./en/Operations.md), [Webhook (English)](./en/Webhook-Forwarding.md)
- [CLI compatible sendmail](./fr/Sendmail.md)

---

## Italiano

📖 **[Leggi la documentazione in italiano →](./it/README.md)**

### Documenti disponibili

- [OwlMail × MailDev × MailCatcher × Mailpit: guida a funzionalità, API e migrazione](./it/Comparison-and-Migration.md)
- [Registro della Refactorizzazione dell'API](./it/internal/API_Refactoring_Record.md)
- Riferimenti condivisi: [Note di rilascio 0.10.0 (English)](./en/Release-0.10.0.md), [API (English)](./en/API-Reference.md), [OpenAPI JSON](../openapi/openapi.json) / [YAML](../openapi/openapi.yaml), [Operations (English)](./en/Operations.md), [Webhook (English)](./en/Webhook-Forwarding.md)
- [CLI compatibile con sendmail](./it/Sendmail.md)

---

## 日本語

📖 **[日本語でドキュメントを読む →](./ja/README.md)**

### 利用可能なドキュメント

- [OwlMail × MailDev × MailCatcher × Mailpit: 機能・API・移行ガイド](./ja/Comparison-and-Migration.md)
- [API リファクタリング記録](./ja/internal/API_Refactoring_Record.md)
- 共通リファレンス：[0.10.0 リリースノート (English)](./en/Release-0.10.0.md)、[API (English)](./en/API-Reference.md)、[OpenAPI JSON](../openapi/openapi.json) / [YAML](../openapi/openapi.yaml)、[Operations (English)](./en/Operations.md)、[Webhook (English)](./en/Webhook-Forwarding.md)
- [sendmail 互換 CLI](./ja/Sendmail.md)

---

## 한국어

📖 **[한국어로 문서 읽기 →](./ko/README.md)**

### 사용 가능한 문서

- [OwlMail × MailDev × MailCatcher × Mailpit: 기능, API 및 마이그레이션 가이드](./ko/Comparison-and-Migration.md)
- [API 리팩토링 기록](./ko/internal/API_Refactoring_Record.md)
- 공통 참조: [0.10.0 릴리스 노트 (English)](./en/Release-0.10.0.md), [API (English)](./en/API-Reference.md), [OpenAPI JSON](../openapi/openapi.json) / [YAML](../openapi/openapi.yaml), [Operations (English)](./en/Operations.md), [Webhook (English)](./en/Webhook-Forwarding.md)
- [sendmail 호환 CLI](./ko/Sendmail.md)

---

## 📚 Documentation Structure

Documents are organized by language in separate directories:

```
docs/
├── README.md (this file - main index)
├── en/          (English)
├── zh-CN/       (简体中文)
├── de/          (Deutsch)
├── fr/          (Français)
├── it/          (Italiano)
├── ja/          (日本語)
└── ko/          (한국어)
```

Each language directory contains:
- `README.md` - Documentation index for that language
- Translated documents and links to canonical English/Chinese release and operational guides
- `internal/` subdirectory - Historical implementation records, not the stable API contract

## 🔄 Contributing

We welcome contributions to translate missing documents. Please see the [Contributing Guide](../.github/CONTRIBUTING.md) for more information.

When adding new documentation:

1. Create the English version first in the `en/` directory.
2. Add translations in the corresponding language directories.
3. Update all language README files to include links to new documents.
4. Follow the existing directory structure (documents in language directories, no language suffix in filenames).

---

For more information about OwlMail, please visit the [main README](../README.md).
