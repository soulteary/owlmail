...
OwlMail 0.9.0 は、既定で無効の読み取り専用 MCP インターフェースを Streamable HTTP と stdio で提供します。MailDev 3 の MCP はより広範で、MailCatcher に組み込み MCP はありません。既定で無効の `-maildev-rest-compat` を有効にすると、現在の MailDev REST
