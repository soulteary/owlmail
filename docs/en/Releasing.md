...
annotated semantic-version tag such as `v0.10.0`. GitHub release binaries and
...
For 0.10.0, also confirm that browser `Origin` validation is exercised against
the Web, API, WebSocket, and MCP surfaces including the documented `*` opt-out,
that a configuration whose SMTPS listener cannot bind fails to start instead of
starting without it, that the fuzz seed corpora replay clean, and that every
current-version example is derived from `VERSION` or has been updated before
tagging.
...
git status --short
git rev-parse HEAD
git tag -a v0.10.0 -m "OwlMail v0.10.0"
...
```bash
gh workflow run release.yml --ref v0.10.0 -f version=v0.10.0
```
...
For `v0.10.0`, verify the `0.10.0`, `0.10`, and `0` tags and both target
...
```bash
docker buildx imagetools inspect ghcr.io/soulteary/owlmail:0.10.0
cosign verify \
  --certificate-identity-regexp '^https://github.com/soulteary/owlmail/.github/workflows/release.yml@refs/tags/v' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
...
- [ ] GitHub marks `v0.10.0` as the latest non-prerelease release.
...
- [ ] Go installation with `@v0.10.0` succeeds on the documented Go version.
