...
OwlMail은 캡처된 HTML 이메일을 신뢰할 수 없는 콘텐츠로 취급합니다. 서버가 능동
콘텐츠를 제거하고 Web UI는 sandbox 권한이 없는 `srcdoc` iframe,
`referrerpolicy="no-referrer"`, 제한적인 CSP로 결과를 표시합니다.

원격 이미지, 글꼴, 스타일시트와 미디어는 기본적으로 차단됩니다. **원격 콘텐츠
로드**는 현재 메시지에만 적용되며 발신자가 선택한 인프라에 접속해 IP 주소와 추적
식별자를 노출할 수 있습니다. CID 이미지는 로컬 첨부 파일로 유지됩니다.
