...
func TestOutgoingMailUpdateConfig(t *testing.T) {
	...
	if config.Host != newConfig.Host {
		t.Errorf("Expected host %s, got %s", newConfig.Host, config.Host)
	}
	if config.Port != newConfig.Port {
		t.Errorf("Expected port %d, got %d", newConfig.Port, config.Port)
	}
	if config.User != newConfig.User {
		t.Errorf("Expected user %s, got %s", newConfig.User, config.User)
	}
	if config.Secure != newConfig.Secure {
		t.Errorf("Expected secure %v, got %v", newConfig.Secure, config.Secure)
	}
	if config.AutoRelay != newConfig.AutoRelay {
		t.Errorf("Expected autoRelay %v, got %v", newConfig.AutoRelay, config.AutoRelay)
	}
	if !om.enabled {
		t.Error("enabled should be true when host is set")
	}
