...
`v0.10.0`은 현재 안정 버전이자 보안 릴리스입니다. Web UI, REST API, WebSocket
스트림, 읽기 전용 MCP 엔드포인트가 이제 기본적으로 브라우저 `Origin` 헤더를
검증합니다. 수집된 메일은 더 이상 모두가 목록을 볼 수 있는 디렉터리에 기록되지
않으며, 암시적 TLS 리스너 포트는 바인딩하지 못하는 경우가 잦던 특권 포트에
고정되지 않고 설정 가능해졌습니다. runtime API와 저장 형식은 0.9.0과 호환되지만
업그레이드 시 보이는 변경이 세 가지 있습니다. 배포 전에 릴리스 노트를 확인하세요.

아래 예제는 `ghcr.io/soulteary/owlmail:0.10.0`으로 고정됩니다.
재현 가능한 CI에서는 전체 버전 또는
`ghcr.io/soulteary/owlmail@sha256:<digest>`를 사용하세요.
[0.10.0 릴리스 노트](./docs/en/Release-0.10.0.md)를 참고하세요.
...
**컨테이너 포트는 항상 루프백으로만 게시하세요. `-p 1080:1080`이 아니라
`-p 127.0.0.1:1080:1080`으로 씁니다.** 이미지는 `OWLMAIL_WEB_HOST=0.0.0.0`과
`OWLMAIL_SMTP_HOST=0.0.0.0`을 설정하며, 이는 컨테이너 내부에서는 올바른 설정입니다
— 프로세스는 자신의 네트워크 네임스페이스 밖에서 오는 연결을 받아야 하기 때문입니다
— 하지만 그만큼 누가 OwlMail에 도달할 수 있는지는 오직 `-p`가 결정합니다. 짧은
형식은 호스트의 모든 인터페이스에 게시하며, 대부분의 사람이 작성해 둔 호스트 방화벽
규칙으로는 막히지 않습니다. 게시된 포트는 포워딩을 거쳐 도달하므로 `ufw deny 1080`
같은 `INPUT` 체인 규칙은 적용되지 않습니다. 이를 걸러내려면 Docker의 `DOCKER-USER`
체인에 규칙을 넣거나 nftables 또는 firewalld에서 같은 설정을 해야 합니다.
그 결과 비밀번호 재설정 링크와 인증 코드, 토큰이 담긴
메일함과 기본적으로 인증 없이 메일을 받는 SMTP 포트가 노출됩니다. 아래 예제는 모두
루프백 형식을 사용합니다.
...
**사용 가능한 태그:**
- `0.10.0` - 정확한 릴리스 태그. `0.10`와 `0`은 해당 계열의 후속 릴리스에 따라 이동
- `sha-<commit>` - 특정 짧은 커밋 SHA의 이미지(예: `sha-8d3445d`)
...
| `-web-allowed-origins` | `OWLMAIL_WEB_ALLOWED_ORIGINS` | - | OwlMail 자체 출처 외에 웹 UI와 REST API에서 허용하는 브라우저 출처. `*`는 출처 검증을 끕니다 |
...
| `-smtps-port` | `OWLMAIL_SMTPS_PORT` | 465 | TLS 활성화 시 암시적 TLS(SMTPS) 리스너 포트; `0` 이면 SMTPS 리스너를 시작하지 않음 |
...
Basic Auth 및 브라우저 Origin 가드 오류는 API 핸들러보다 먼저
...
For detailed migration guide, see: [OwlMail × MailDev × MailCatcher × Mailpit: 기능, API 및 마이그레이션 가이드](./docs/ko/Comparison-and-Migration.md)
...
- [OwlMail 0.10.0 릴리스 노트](./docs/en/Release-0.10.0.md) ([中文](./docs/zh-CN/Release-0.10.0.md))
...
- [변경 기록](./CHANGELOG.md)
- [OwlMail × MailDev × MailCatcher × Mailpit: 기능, API 및 마이그레이션 가이드](./docs/ko/Comparison-and-Migration.md)
